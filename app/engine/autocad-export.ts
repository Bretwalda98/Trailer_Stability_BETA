...
export const AUTOCAD_EXPORT_KEY = {
  ...
  sections: [
    ...
    { code: "su", name: "supports", fields: { id: "support id", x: "X position", w: "width", al: "allowed", ac: "active input", pc: "positive connection to deck/spine beam", ra: "settled active", rt: "reaction", rs: "reaction state", dr: "disable reason" } },
    ...
    { code: "r", name: "calculation result", fields: { st: "status", fc: "fail class", fd: "fail detail", tm: "total mass", lc: "load COG", cc: "combined COG", gp: "hydraulic groups", ax: "axle points", sx: "spine axle points", ss: "settled supports", se: "complete support-settlement trace", ov: "overlap pairs", gq: "grouping quality", pg: "stability polygon", cp: "basic/slope/dynamic case points", sr: "stability references", an: "controlling analysis", rt: "road transport result", bm: "beam metrics", mt: "all metric values", gb: "ground-bearing summary and per-group values", hp: "neutral and A-D hydraulic pressure cases", sm: "drawing-table mass and loading summary", ws: "warnings", ms: "calculation milliseconds", rv: "resolved trailer geometry used by the authoritative result" } },
  ...
  return {
    ...
    data: {
      ...
      su: model.supports.map((support, index) => {
        ...
        return { id: support.id, x: support.xM, w: support.widthM, al: support.allowed, ac: support.active, pc: support.positiveConnectionToDeck === true, ra: settled?.active ?? false, rt: settled?.reactionT ?? null, rs: settled?.reactionState ?? "UNAVAILABLE", dr: settled?.disableReason ?? "" };
      }),
      en: {
