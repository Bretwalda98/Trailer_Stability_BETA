    ...
    <dialog ref={dialogRef} className="setup-wizard-dialog arrangement-wizard-dialog" aria-labelledby="arrangement-wizard-title" onCancel={(event) => { event.preventDefault(); onClose(); }}>
      <div className={`setup-wizard-shell optimiser-wizard-shell arrangement-wizard-shell${previewExpanded ? " preview-expanded" : ""}`}>
        ...
        <section className="setup-wizard-preview arrangement-wizard-preview" aria-label="Mathematical arrangement plan">
          ...
          {step !== "cargo" && step !== "packing" && <><div className="arrangement-preview-hero"><span>MINIMUM CAPACITY RECOMMENDATION</span><b>{quickRecommendations.capacityRecommendation?.totalAxleLines ?? quickRecommendations.capacityLowerBoundAL} <small>total AL</small></b><p>{quickRecommendations.capacityRecommendation ? <><b>{quickRecommendations.capacityRecommendation.trainCount} train{quickRecommendations.capacityRecommendation.trainCount === 1 ? "" : "s"} × {quickRecommendations.capacityRecommendation.axleLinesPerTrain} AL/train</b> · {moduleText(quickRecommendations.capacityRecommendation.modules4, quickRecommendations.capacityRecommendation.modules5, quickRecommendations.capacityRecommendation.modules6)}</> : "No constructible train/module combination is available within the current bounds."}</p><p>{quickRecommendations.payloadMassT.toFixed(2)} t cargo + packing payload needs at least {quickRecommendations.payloadOnlyLowerBoundAL} AL at the configured gross utilisation limit. The conservative engineering bound is {quickRecommendations.capacityLowerBoundAL} AL before constructible 4/5/6-AL module rounding, retaining trailer tare and selected PPUs.</p></div>
